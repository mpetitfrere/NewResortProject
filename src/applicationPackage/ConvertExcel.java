...
public class ConvertExcel {
	...
	public static void exportExcel(boolean templateState) throws IOException {
		...
		if (templateState == true) {// When Template is chosen only print
									// columns
		...
		for (int rows = 0; rows < model.getRowCount(); rows++) { // For each
			...
			for (int cols = 0; cols < table.getColumnCount(); cols++) { // For
				...
				if (table.getColumnName(cols).equals(colName[cols])) {
					...
					if (!isColumnIntType(columnString)) {// writes cell as float
															// type to remove
															// error checking in
															// excel
						XSSFCell cell = (XSSFCell) row.createCell(cols);// create
																		// a
																		// cell
																		// at
																		// the
						...
						// row.createCell(cols).setCellValue(model.getValueAt(rows,
						// cols).toString()); //Write value
					} else// writes the cell as strings
					{
						XSSFCell cell = (XSSFCell) row.createCell(cols);// create
																		...
																		// row,col
																		// location
						String x = (String) (model.getValueAt(rows, cols));// get
																			// he
																			// value
																			// from
																			// table
			...
			for (int cols = 0; cols < table.getColumnCount(); cols++) { // For
				...
				if (table.getColumnName(cols).equals(colName[cols])) {
					...
					if (!isColumnIntType(columnString)) {// writes cell as float
															...
															// error checking in
															// excel
						XSSFCell cell = (XSSFCell) (headerRow).createCell(cols);// create
																				// a
																				// cell
																				// at
																				// the
	...
	public static boolean isColumnIntType(String colName) {
		if (colName.equals("AssociationName") || (colName.equals("Type")) || (colName.equals("Depth"))) {
			return true;
		}
		return false;
	}
	...
	/**
	 * Verifies File import meets sqlite's standard by checking if the columns
	 * match
	 */
	public static boolean validateExcel(File fs) throws SQLException {
		...
		for (int count = 0; count < c.length; count++) {// get column headers
														// from excel
	...
	public static String excelName() {
		...
		if (!theDir.exists()) {
			...
			// System.getProperty("user.dir") + "/Excel");
		...
		for (int i = 0; i < listOfFiles.length; i++) {
			...
			if (fileName.equals(listOfFiles[i].toString())) {
				file = new File("Excel\\\\Form" + "(" + (i) + ")" + ".xlsx");// need
																				// to
																				// change
																				// i
																				// to
	...
	public static void UpDateTable(JTable table) {// Duplicate
		try {
			Connection conn = MySQLConnection.dbConnector();
	 ...
	 * @param row
	 */
	public static PreparedStatement getParepareValues(Cell[] cellArray, int j, PreparedStatement prepare, int row)
			throws SQLException {
		...
		if (cellArray[j].getCellType() == Cell.CELL_TYPE_STRING && (j == 0 || j == 2 || j == 6)) {
			try {
				if (j == 6 && !(cellArray[j].getStringCellValue().equals("F")
					...
					JOptionPane.showMessageDialog(null,
							"Column Depth(G) at row: " + row
									+ " does not contain F or B , please fix this and reimport.",
			...
			try {
				...
				if (String.valueOf(cellInt).length() != 4 && j == 1) {
					JOptionPane.showMessageDialog(null,
							"Column Year(B) at row: " + row + " Year must be ented in this format: YYYY ", "Error",
							JOptionPane.ERROR_MESSAGE);
					return prepare;
				} else if ((cellInt <= 0 || cellInt > 6) && j == 3) {
					JOptionPane.showMessageDialog(null,
							"Column Aisle(D) at row: " + row + " Aisle must be between 1 and 6", "Error",
							JOptionPane.ERROR_MESSAGE);
					return prepare;
				}
					...
					JOptionPane.showMessageDialog(null,
							"Column Row(E) at row: " + row + " .Row must be between 1 and 10", "Error",
							JOptionPane.ERROR_MESSAGE);
					return prepare;
				} else if ((cellInt <= 0 || cellInt > 30) && j == 5) {
					JOptionPane.showMessageDialog(null, "Column(F) at row: " + row + " must be between 1 and 30",
							"Error", JOptionPane.ERROR_MESSAGE);
					return prepare;
				} else {
	...
	public static boolean importExcel(File fs) throws SQLException {
		...
		for (int i = 1; i <= sheet.getLastRowNum(); i++) {// start at 1 to skip
															// column
			row = sheet.getRow(i);// change colcounts to row
			int colCounts = columnLength;// assign colCounts to the length of
											// max num of cols
			for (int j = 0; j < size; j++) {
				...
				if (!isCellEmpty(cellArray[j])) {
					...
					JOptionPane.showMessageDialog(null,
							"There is an empty cell, please fill with" + " the appropriate value and reimport.",
							"ERROR", JOptionPane.ERROR_MESSAGE);
					return false;
				}
